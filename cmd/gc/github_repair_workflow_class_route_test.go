...
// monitor does: a PR bead already on the work ledger, and a workflow the
// monitor's config names.
func attachRepairWorkflow(t *testing.T, cityDir string, work beads.Store, workflow string) beads.Bead {
	...
	if err := defaultAttachGitHubPRRepairWorkflow(cityDir, work, cfg, rig, monitor, parent, result); err != nil {
